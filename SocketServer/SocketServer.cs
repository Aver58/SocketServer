...
{
    #region Instance
    private class InstanceHolder { public static SocketServer Instance = new SocketServer(); }
    public static SocketServer Instance { get { return InstanceHolder.Instance; } }
    #endregion
    ...
    public void StartServer(string ip = "127.0.0.1", int port = 8090)
    {
        ...
        //开始接受客户端连接请求
        socket.BeginAccept(new AsyncCallback(OnOneClientAccepted), socket);

        //Run(ip, port);

        Console.WriteLine("Server Start Listen on: " + ip + ":" + port.ToString());
        Console.Read();
    }
