...
namespace Server.Framework.Service {
    class ServiceContext {
        ...
        public void Push(Message msg) {
            ...
            try {
                ...
                if (!m_isInGlobal) {
                    GlobalMQ.Instance.Push(m_serviceAddress);
                    m_isInGlobal = true;
                }
            } finally {
                if (isLock)
    ...
    class Message {
        ...
        public int Source { get; set; } //指定发送请求的服务，如果不是由服务发送则填0
        public int Destination { get; set; } //目标服务ID，被请求服务的ID
        public int RPCSession { get; set; } //发送者可能发起RPC请求，因此接受者返回消息时，需要将RPCSession信息带回给发送者，方便其找回对应请求的环境，并进行响应
